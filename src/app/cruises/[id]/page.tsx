...
const getAmenityIcon = (name: string) => {
  ...
  if (nameLower.includes("rượu") || nameLower.includes("wine") || nameLower.includes("cigar")) return <Wine className="w-5 h-5 text-accent" />;
  ...
  if (nameLower.includes("buffet") || nameLower.includes("tôm hùm") || nameLower.includes("ẩm thực")) return <Utensils className="w-5 h-5 text-accent" />;
  if (nameLower.includes("nhạc") || nameLower.includes("music") || nameLower.includes("piano")) return <Music className="w-5 h-5 text-accent" />;
...
export default function CruiseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (selectedCabinIdx !== null || isBookingModalOpen) {
  ...
  }, [selectedCabinIdx, isBookingModalOpen]);
  ...
  const handleCabinsScroll = () => {
    const container = cabinsScrollRef.current;
    if (!container) return;
    ...
    if (totalItemWidth > 0) {
      ...
      setActiveCabinIdx(index);
    }
  };

  const scrollToCabin = (index: number) => {
    const container = cabinsScrollRef.current;
    if (!container) return;
  ...
  const matchedReviews = useMemo(() => {
    ...
    const matched = mockReviews.filter(
      (r) => r.stayType.toLowerCase().includes(cruise.name.split(" ")[0].toLowerCase())
    );
    ...
    return (
      <>
        ...
        <div className="min-h-[70vh] flex flex-col items-center justify-center bg-white px-6">
          ...
          <Link href="/cruises" className="px-6 py-2.5 bg-[#001226] text-white hover:bg-accent hover:text-[#001226] text-xs uppercase tracking-widest font-bold rounded-full transition-all">
            Quay lại danh sách
          </Link>
        </div>
        <Footer />
  ...
  const mockCabins = [
    {
      ...
      size: "32 m²",
      capacity: "2 Khách",
      description: "Hạng phòng Deluxe tiêu chuẩn được bố trí tại tầng 1 với cửa kính lớn chạm trần và ban công riêng biệt hướng ra vịnh. Trang bị đầy đủ tiện nghi hiện đại và phòng tắm đứng sang trọng.",
    ...
    },
    {
      ...
      size: "48 m²",
      capacity: "3 Khách",
      description: "Tọa lạc tại tầng 2 của du thuyền với góc nhìn toàn cảnh 180 độ tuyệt mỹ ra vịnh. Hạng phòng được thiết kế theo phong cách Indochine thượng lưu, bồn tắm nằm cao cấp sát cửa kính ngắm cảnh.",
      ...
      amenities: ["Ban công kính panorama", "Bồn tắm jacuzzi riêng", "Dịch vụ trà chiều tại phòng", "Trái cây tươi mỗi ngày", "Mini bar free refill"]
    },
    {
      name: "Presidential Royal Suite",
      size: "95 m²",
      capacity: "4 Khách",
      description: "Căn phòng rộng lớn và vương giả bậc nhất trên tàu, sở hữu phòng khách, bàn ăn và boong tắm nắng sundeck riêng tư hoàn toàn. Đi kèm dịch vụ quản gia cá nhân 24/7 phục vụ mọi bữa ăn tại phòng.",
      ...
      amenities: ["Sundeck riêng biệt lập", "Phòng khách sang trọng", "Quản gia phục vụ 24/7", "Đón tiễn VIP limousine", "Rượu champagne chào mừng"]
    },
    {
      name: "Grand Owner's Penthouse Suite",
      size: "120 m²",
      capacity: "4 Khách",
      description: "Căn phòng tổng thống sang trọng bậc nhất với hồ bơi jacuzzi vô cực riêng, phòng tắm sauna hơi nước khô, bàn ăn lớn phục vụ tiệc và tầm nhìn trực diện mũi tàu không góc chết.",
      ...
      amenities: ["Bể bơi vô cực mũi tàu", "Phòng xông hơi riêng", "Quản gia trưởng 24/7", "Đồ uống cao cấp free", "Massage trị liệu miễn phí"]
    },
    {
      name: "Family Connecting Suite",
      size: "64 m²",
      capacity: "5 Khách",
      description: "Lựa chọn hoàn hảo cho kỳ nghỉ gia đình với thiết kế 2 phòng ngủ thông nhau qua cửa kết nối riêng biệt. Đầy đủ tiện nghi cho trẻ em và khu sinh hoạt chung đầm ấm.",
  ...
  const mockActivities = [
    {
      ...
      description: "Trải nghiệm tinh hoa ẩm thực 5 sao với tiệc tối buffet tôm hùm, Fine Dining phong cách Á-Âu được chế biến bởi các bếp trưởng hàng đầu, kết hợp cùng bộ sưu tập rượu vang thượng hạng.",
    ...
    },
    {
      ...
      description: "Thư giãn cùng những ly cocktail độc bản tại Sky Lounge lộng gió hay hầm rượu cigar sang trọng dưới tầng hầm, lắng nghe tiếng sóng vỗ rì rào hòa cùng bản nhạc jazz êm dịu.",
    ...
    },
    {
      ...
      title: "Spa & Trị liệu",
      description: "Hồi phục sức khỏe Thân - Tâm - Trí với các liệu trình massage đá nóng, xông hơi đá muối Himalaya tại khu vực spa biệt lập, được phục vụ bởi các kỹ thuật viên chuyên nghiệp.",
    ...
    },
    {
      ...
      label: "Swimming pool",
      title: "Hồ bơi vô cực",
      description: "Thỏa sức bơi lội giữa bể bơi bốn mùa nước mặn ngoài trời, nhâm nhi ly nước ép trái cây tươi mát bên ghế tắm nắng ngắm hoàng hôn buông xuống vịnh di sản.",
    ...
    },
    {
      ...
      description: "Đắm chìm trong các hoạt động biểu diễn nghệ thuật truyền thống, chiếu phim ngoài trời trên boong tàu hay thử tài câu mực đêm cùng các thủy thủ đoàn vô cùng thú vị.",
  ...
  const handlePrevPhoto = () => {
    setActivePhotoIdx((prev) => (prev === 0 ? cruise.imageGallery.length - 1 : prev - 1));
  };

  const handleNextPhoto = () => {
    setActivePhotoIdx((prev) => (prev === cruise.imageGallery.length - 1 ? 0 : prev + 1));
  };
  ...
  return (
    <div className="w-full bg-white text-slate-800">
      ...
      {/* 1. HERO BANNER - DYNAMIC OVERLAY (Ponant L'Austral Style) */}
      <section className="relative h-[80vh] w-full bg-slate-900 overflow-hidden">
        ...
        {/* Breadcrumb Path absolute inside banner (like list pages) */}
        <div className="absolute top-28 left-0 right-0 z-20">
          <div className="max-w-7xl mx-auto px-6 flex items-center gap-2 text-[10px] text-white/60 uppercase tracking-widest font-semibold">
            ...
            <Link href="/cruises" className="hover:text-accent">Du thuyền</Link>
            <ChevronRight className="w-3 h-3 text-white/40" />
            <span className="text-white/95">{cruise.name}</span>
          </div>
        </div>
          ...
          <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-start md:items-end justify-between gap-6 w-full">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-0.5 text-accent">
                    {Array.from({ length: 5 }).map((_, index) => (
                      <Star
                        key={index}
                        className={`w-4 h-4 ${index < cruise.stars ? "fill-accent text-accent" : "text-white/20"}`}
                      />
                    ))}
                  </div>
                  <span className="text-xs font-semibold text-white/80 tracking-wider">
                    ({cruise.stars}.0) • {cruise.destinations[0]}
                  </span>
                </div>
              </div>
            </div>

            {/* Right Column: Dynamic Booking Button (Never wraps, never squished) */}
            <div className="flex items-center gap-6 flex-shrink-0 whitespace-nowrap">
              <div className="text-right hidden sm:block">
                ...
                <span className="text-xl font-bold text-accent font-serif">{formatPrice(cruise.priceFrom / 2)}</span>
                <span className="text-[10px] text-white/40 block">/ khách</span>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-16 items-start">
          ...
          <div className="lg:col-span-2 space-y-12">
            ...
            <div className="space-y-4">
              <span className="text-[10px] uppercase tracking-[0.25em] text-accent-dark font-bold block">Tổng quan du hành</span>
            ...
            <div className="space-y-6 pt-6 border-t border-slate-100">
              <div className="space-y-1">
                <span className="text-[10px] uppercase tracking-[0.25em] text-accent-dark font-bold block">Trải nghiệm cao cấp</span>
                <h3 className="font-serif text-lg font-bold text-slate-900">TIỆN ÍCH NỔI BẬT</h3>
              </div>
            ...
            <div className="space-y-6 pt-6 border-t border-slate-100">
              <div className="space-y-1">
                <span className="text-[10px] uppercase tracking-[0.25em] text-accent-dark font-bold block">Quyền lợi đặc biệt</span>
                <h3 className="font-serif text-lg font-bold text-slate-900">ĐẶC QUYỀN ĐI KÈM</h3>
              </div>
          ...
          {/* Right Side: Technical Specs (col-span-1) */}
            ...
            <div className="bg-slate-50 border border-slate-200/60 p-8 rounded-sm space-y-6 shadow-sm">
              ...
              <div className="space-y-4 text-xs">
                ...
                <div className="flex justify-between py-2 border-b border-slate-100">
                  ...
                  <span className="font-bold text-slate-800 uppercase text-right max-w-[180px]">{cruise.material}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-slate-100">
                  <span className="text-slate-500 font-medium">SỐ CABINS PHÒNG</span>
      ...
      <section className="py-20 bg-slate-50 border-t border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-6 space-y-8">
          ...
          {/* Real-Life Gallery Slider (Image 1 Style) */}
          <div className="relative w-full h-[400px] md:h-[550px] overflow-hidden rounded-sm bg-slate-900 shadow-lg group">
            <img
              src={cruise.imageGallery[activePhotoIdx]}
              alt={`Cruise view ${activePhotoIdx + 1}`}
              className="w-full h-full object-cover transition-all duration-700 ease-in-out"
            />
            {/* Soft dark overlay at the bottom for readability */}
            <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/60 to-transparent" />

            {/* Bottom-left: Dot Pagination */}
            <div className="absolute bottom-6 left-6 z-10 flex items-center gap-1.5">
              {cruise.imageGallery.map((_, idx) => (
                <button
                  key={idx}
            ...
            <div className="absolute bottom-6 right-6 z-10 flex items-center gap-3">
              <button
                ...
                className="w-10 h-10 rounded-full border border-white/40 hover:border-white text-white hover:bg-white/10 flex items-center justify-center transition-all cursor-pointer"
      ...
      {/* 4. CABINS CATEGORIES SECTION (Carousel with snap scroll) */}
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6 space-y-12">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-6 border-b border-slate-100 pb-8">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-10 h-10 rounded-full border border-slate-200 hover:border-slate-800 text-slate-650 hover:bg-slate-50 flex items-center justify-center transition-all cursor-pointer bg-white shadow-sm"
          ...
          {/* Cabins Scrollable snap carousel */}
          <div 
            ref={cabinsScrollRef}
            onScroll={handleCabinsScroll}
                  ...
                  <div className="space-y-2.5 border-t border-white/20 pt-3">
                    <div className="flex gap-6 text-[10px] text-white/70">
                      <div>
                        ...
                        <span className="font-bold text-white text-xs">{cabin.capacity}</span>
                      </div>
                      <div>
                        <span className="block text-[8px] text-white/70 uppercase tracking-widest font-semibold">Diện tích</span>
                        <span className="font-bold text-white text-xs">{cabin.size}</span>
                      </div>
                    </div>
                    <div className="flex justify-between items-baseline pt-1">
                      <span className="text-[9px] text-white/70 uppercase tracking-widest font-semibold">Giá từ:</span>
                      <span className="font-serif text-xs md:text-sm font-bold text-accent whitespace-nowrap">{formatPrice(cabin.price)}</span>
                    </div>
                  </div>

                  {/* Discover Capsule Button */}
                  <button
                    onClick={() => {
          ...
          <div className="flex justify-center items-center gap-1.5">
            {Array.from({ length: Math.max(1, mockCabins.length - visibleCount + 1) }).map((_, idx) => (
              <button
                key={idx}
                onClick={() => scrollToCabin(idx)}
                className={`h-1.5 rounded-full transition-all duration-300 cursor-pointer ${
                  idx === activeCabinIdx ? "w-6 bg-[#001226]" : "w-1.5 bg-slate-300 hover:bg-slate-450"
                }`}
                aria-label={`Go to cabin page ${idx + 1}`}
              />
            ))}
          </div>

          {/* Embedded style to hide scrollbars */}
          <style>{`
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-7 flex flex-col md:flex-row items-center justify-between gap-12">
            ...
            <div className="w-full md:w-3/5 space-y-4">
              <span className="text-[10px] text-slate-400 uppercase tracking-[0.2em] block font-semibold">Life on board</span>
      ...
      <section className="py-20 bg-white" id="enquiry-section">
            ...
            <div className="space-y-6 max-h-[500px] overflow-y-auto pr-2 scrollbar-thin">
              {allReviews.map((rev) => (
                <div key={rev.id} className="p-6 border-b border-slate-100 last:border-0 space-y-4">
                  ...
                  <p className="text-xs text-slate-600 leading-relaxed font-sans font-light pl-12">
                    {rev.comment}
                  </p>
                </div>
              ))}
            ...
            >
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 border-b border-slate-100 pb-6">
                  ...
                  <div className="flex gap-8 text-right self-start">
                    <div>
                      ...
                      <span className="font-serif text-xl font-bold text-slate-800">{activeCabin.size}</span>
                    </div>
                    <div>
                      <span className="block text-[10px] text-slate-400 uppercase tracking-widest font-semibold">Khách tối đa</span>
                      <span className="font-serif text-xl font-bold text-slate-800">{activeCabin.capacity}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4 text-left">
                  ...
                  <div className="border-t border-slate-100 pt-6 flex items-center justify-between gap-4">
                    <div className="flex flex-col">
                      <span className="text-[10px] uppercase tracking-widest text-slate-400">Giá phòng từ</span>
                      <span className="font-serif text-lg font-bold text-slate-900">{formatPrice(activeCabin.price)} <span className="text-[10px] text-slate-450 font-sans font-normal">/ đêm</span></span>
                    </div>
                    <button
                      onClick={() => {
              ...
              <div className="border-t border-slate-100 px-6 py-4 bg-slate-50 flex items-center justify-between">
                ...
                >
                  <ChevronRight className="w-4 h-4 rotate-180" />
                  <span>
                    {mockCabins[selectedCabinIdx === 0 ? mockCabins.length - 1 : selectedCabinIdx - 1].name}
                  </span>
                </button>
                ...
                >
                  <span>
                    {mockCabins[selectedCabinIdx === mockCabins.length - 1 ? 0 : selectedCabinIdx + 1].name}
                  </span>
                  <ChevronRight className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="w-full md:w-[350px] bg-slate-50 p-6 md:p-8 border-t md:border-t-0 md:border-l border-slate-200 flex flex-col justify-between overflow-y-auto text-left shadow-inner">
                  ...
                  <div className="space-y-4 text-xs text-slate-650">
                    ...
                    <div className="flex justify-between border-b border-slate-150 pb-2">
                      ...
                      <span className="font-bold text-[#001226] text-right">
                        {bookingDate ? new Date(bookingDate).toLocaleDateString("vi-VN", { day: "numeric", month: "numeric", year: "numeric" }) : "Chưa chọn"}
                      </span>
                    </div>
                          ...
                          {mockCabins.map((cabin, idx) => {
                            ...
                            return (
                              <div key={idx} className="flex justify-between text-[11px]">
                                <span className="font-semibold text-slate-800 max-w-[170px] truncate">{cabin.name} <span className="text-slate-455 font-normal text-[10px]">x{qty}</span></span>
                    ...
                    <div className="flex justify-between text-[11px] border-b border-slate-150 pb-2">
                      <span className="font-medium text-slate-500">Hệ số ngày đi ({bookingItinerary === "3n2d" ? "3N2Đ" : bookingItinerary === "4n3d" ? "4N3Đ" : "2N1Đ"}):</span>
                      <span className="font-semibold text-slate-800">x {bookingItinerary === "3n2d" ? "1.8" : bookingItinerary === "4n3d" ? "2.5" : "1.0"}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-5 pt-6 border-t border-slate-200 mt-6">
                  ...
                  <div className="bg-amber-50 border border-amber-250 p-3 rounded-lg text-[10px] text-amber-800 leading-relaxed font-medium">
                    * Giá báo trên đã bao gồm hệ số hành trình nhưng chưa tính các phụ thu trẻ em hoặc phí dịch vụ cao cấp khác. Tư vấn viên TRAVEL sẽ liên hệ sớm nhất.
                  </div>
